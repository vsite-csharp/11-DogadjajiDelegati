...
namespace Vsite.CSharp.DogađajiDelegati
{
    ...
    class GetInvocationList
    {
        ...
        {
            foreach (Funkcija mojaFunkcija in fja.GetInvocationList())
            {
                Console.WriteLine("Ispis funkcije {0}:", mojaFunkcija.Method);
                Console.WriteLine("{0,15:0.0000000000} {1,20:0.0000000000}", "x", "y");
                for (int i = 0; i < n; ++i)
                {
                    double x = x1 + (x2 - x1) / (n - 1) * i;
                    Console.WriteLine("{0,15:0.0000000000} {1,20:0.0000000000}", x, mojaFunkcija(x));
                }
            }
        ...
        //  Pokrenuti i provjeriti testove (test u grupi "GetInvocationList" mora proći)
